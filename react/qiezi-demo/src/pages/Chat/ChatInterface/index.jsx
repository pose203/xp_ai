    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          setAiTyping(false);
          try { Toast?.fail?.('图片分析失败，请稍后重试'); } catch (_) {}
        }
      }, 1500); // 1.5秒延迟，模拟分析时间
      
    } catch (error) {
      console.error('图片上传失败:', error);
      try { Toast?.fail?.('图片上传失败，请重试'); } catch (_) {}
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      if (!file.type.startsWith('image/')) {
        try { Toast?.fail?.('请选择图片文件'); } catch (_) {}
        return;
      }
      
      // 检查文件大小 (限制为5MB)
      if (file.size > 5 * 1024 * 1024) {
        try { Toast?.fail?.('图片大小不能超过5MB'); } catch (_) {}
        return;
      }
